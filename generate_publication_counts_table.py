...
PREPRINTS_FILE_STR = 'Preprints'          # string that, if file name contains it, marks file as preprints
...
TABLE_HEADERS = [ 'Conferences', 'Journals', 'In Books', 'Editorship', 'Books and Theses', 'Preprints', 'Other']
if ENABLEEXTRAS:
...
en = Lang()
en.TableHeaders = [ '<div align="left"><div class="balken-inproceedings" style="align:left; width:10px; height:10px; display: inline-block;"></div> Conferences </div>', 
...
if ENABLEEXTRAS:
    en.SpecialHeaders = ['<div class="balken-fusion" style="width:10px; height:10px; display: inline-block;"></div> <div style="text-indent:20px;width:10px; height:10px; display: inline-block;"><span title="Fusion: International Conference on Information Fusion">Fusion</span></div>',
...
en.TypeYear = '<div style="width:125px; height:10px; display: inline-block;">Type / Year</div>'
en.Total = '<div align="left">Total</div>'

de = Lang()
de.TableHeaders = [ '<div align="left"><div class="balken-inproceedings" style="width:10px; height:10px; display: inline-block;"></div> Konferenzen </div>', 
...
def parse_bib_files(file_list):
  ...
  for file_name in file_list:
    ...
    for entry in bibdata.entries:
      ...
      if not any("Hanebeck" in name for name in authors) and not "Hanebeck" in authors:
        warnings.warn(f'Publication {entry["ID"]} is not by Hanebeck but by {authors}')
        continue
...
def create_counts_table(output_type, filelist):
  ...
  data = [ [ (len(catalog[year][header]) if (header in catalog[year]) else 0) for year in years ] for header in TABLE_HEADERS ] 
  headerTotals = [ sum(row) for row in data ] 
  yearTotals = [ sum(row[y] for row in data) for y in range(0, numYears) ] 

  fullData = [ [ (len(catalog[year][header]) if (header in catalog[year]) else 0) for year in years ] for header in FullHeaders ] 
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description="Generate publication counts from BibTeX files.")
  parser.add_argument("--output_type", choices=["full", "simple"], default="full", help="Choose output type: full or simple.")
